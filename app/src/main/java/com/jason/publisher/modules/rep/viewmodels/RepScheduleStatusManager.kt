    ...
    fun checkScheduleStatus() {
        ...
        if (activity.viewModel.forceAheadStatus && !activity.viewModel.hasPassedFirstStop) {
            try {
                ...
                FileLogger.e("RepScheduleStatusManager", "Error updating 'Please wait' status | ${e.javaClass.simpleName}: ${e.message} | ${TripStateSnapshot.describe()}\n${Log.getStackTraceString(e)}")
                activity.runOnUiThread {
                    try {
                        binding.scheduleStatusValueTextView.text = "Please wait..."
                    } catch (e2: Exception) {
                        FileLogger.e("RepScheduleStatusManager", "Error in fallback | ${e2.javaClass.simpleName}: ${e2.message} | ${TripStateSnapshot.describe()}\n${Log.getStackTraceString(e2)}")
                    }
                }
            }
            return
        ...
        try {
                    ...
                    try {
                        ...
                        if (iconView != null) {
                            ...
                            try {
                                ...
                                FileLogger.e("RepScheduleStatusManager", "Error accessing icon | ${e3.javaClass.simpleName}: ${e3.message} | ${TripStateSnapshot.describe()}\n${Log.getStackTraceString(e3)}")
                            }
                        }
                    } catch (e2: Exception) {
                        FileLogger.e("RepScheduleStatusManager", "Error in UI update | ${e2.javaClass.simpleName}: ${e2.message} | ${TripStateSnapshot.describe()}\n${Log.getStackTraceString(e2)}")
                    }
                }
            } catch (e: Exception) {
                FileLogger.e("RepScheduleStatusManager", "Error updating UI | ${e.javaClass.simpleName}: ${e.message} | ${TripStateSnapshot.describe()}\n${Log.getStackTraceString(e)}")
            }
            ...
            FileLogger.e("RepActivity checkScheduleStatus", "Error computing schedule status | ${e.javaClass.simpleName}: ${e.message} | ${TripStateSnapshot.describe()}\n${Log.getStackTraceString(e)}")
        }
    }
